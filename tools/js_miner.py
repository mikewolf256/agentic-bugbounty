...
def collect_js_from_page(base_url, html):
    ...
    for m in re.finditer(r'<script[^>]+src=["\']([^"\']+)["\']', html, flags=re.I):
        src = m.group(1)
        found.add(src)
    # heuristic: urls in JS-like contexts
    for m in re.finditer(r'https?://[^"\'\s]+\.js', html):
        found.add(m.group(0))
    return list(found)

def parse_endpoints_from_js(js_text, base_url):
    # naive: find strings that look like /api/... or https://host/...
    endpoints = set()
...
def run(base_url, outdir):
    ...
    # also try to grab common JS paths
